...
class CrudModel
{
    ...
    public function readData($table, $get_columns, $join = NULL, $whereas = NULL, $group_by = NULL, $order_by = NULL, $like = NULL, $or_like = NULL, $start = NULL, $limit = NULL, $outputType = NULL){
        $this->db->select($get_columns);
        
		$this->db->from($table);
		if(!empty($join)){
			foreach ($join as $join_table => $value1) {
		...
		$query=$this->db->get();
		if($outputType <> null){
			if($outputType=="row_array"){
				return $query->row_array();
			}elseif($outputType=="result_array"){
				return $query->result_array();
			}elseif($outputType=="row_object"){
				return $query->row();
			}elseif($outputType=="result_object"){
				return $query->result();
			}
		}else{
			return $query->result_array();
    ...
    public function getAnyItems($db_data)
    {
        $builder = $this->db->table($db_data['table']);
        if(isset($db_data['select'])){
            $builder->select($db_data['select']);
        }

        if(isset($db_data['join'])){
			foreach ($db_data['join'] as $join_table => $value1) {
				foreach ($value1 as $cond => $join_type) {
                    $builder->join($join_table, $cond, $join_type);
				}
			}
		}
        
        if (isset($db_data['where'])){
            $builder->where($db_data['where']);
        }

        if (isset($db_data['or_where'])){
            $builder->orWhere($db_data['or_where']);
        }
        
        if (isset($db_data['order_by'])){
            $builder->orderBy($db_data['order_by']['column'], $db_data['order_by']['order']);
        }

        if(isset($db_data['output'])){
            if($db_data['output'] == 'result_array'){
                return $builder->get()->getResultArray();
            }elseif($db_data['output'] == 'row_array'){
                return $builder->get()->getRowArray();
            }elseif($db_data['output'] == 'row_object'){
    ...
    {
        $is_update = $this->db->table($table)->where($where)->update($data);
    ...
    public function countTotalRows($table, $where = null)
    {
        $count_query = "SELECT COUNT(*) as total FROM vslink_tbl" . $table;
    ...
    public function nextHostelCode(){
        $count_query = "SELECT MAX(HostelCode) as total FROM vslink_tblhostels";
        $max = $this->db->query($count_query)->getRow()->total;
        return $max == 0 ? sprintf("%04d", 1) : sprintf("%04d", $max + 1);
    }

    public function get_link($link_alias){
        $sql = $this->db->query("SELECT * FROM vslink_tblshortlinks WHERE BINARY alias = '".$link_alias."'");
